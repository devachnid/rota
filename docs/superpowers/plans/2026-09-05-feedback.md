...
```html
<main class="main">{% block content %}{% endblock %}</main>
{# The one htmx modal target on the page: the grid's cell, note and locum
   forms and the Feedback form all render into it. #modal:empty is hidden. #}
...
def test_the_list_and_change_form_render_and_there_is_no_add_page(admin_client, report):
    ...
    assert admin_client.get(_change(report)).status_code == 200
...
def test_a_vanished_reporter_has_no_button_and_a_smuggled_one_sends_nothing(configured, admin_client, report, gp_user):
    gp_user.delete()
    report.refresh_from_db()
    assert "Send reply" not in admin_client.get(_change(report)).content.decode()
